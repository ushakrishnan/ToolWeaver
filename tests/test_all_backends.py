...
def test_backend(backend_name: str) -> bool:
    ...
    try:
        ...
        print(f"[PASS] Recorded 3 executions (2 success, 1 failure)")
        
        # Record ratings
        print(f"[INFO] Recording ratings...")
        if backend_name == "sqlite":
        ...
        print(f"[PASS] Recorded 2 ratings (5 and 4 stars)")
        
        # Update health score
        print(f"[INFO] Updating health score...")
        client.update_health_score(f"test_skill_{backend_name}", score=82.5)
        print(f"[PASS] Updated health score to 82.5/100")
...
def test_otlp_backend() -> bool:
    ...
    if result:
        ...
        time.sleep(62)
        print(f"[INFO] OTLP metrics should now be visible in Grafana Cloud")
...
def test_factory_function() -> bool:
    ...
    print(f"\n{'='*70}")
    print(f"Testing Factory Function")
    print(f"{'='*70}")
